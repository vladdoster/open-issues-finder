...
def main():
    ...
    send_email(cfg, multiprocess_links(gen_gh_interests_urls(cfg=cfg, interests=interests, labels=labels)))
...
def gen_gh_interests_urls(cfg, interests, labels):
    ...
    try:
        return [base_link.format(random.choice(interests), random.choice(labels)) for _ in itertools.repeat(None, int(cfg['User']['projects_limit']))]
    except ValueError:
...
def get_gh_link(url):
    ...
    try:
        ...
        logger.error("Uh-oh, {0!r} experienced an unexpected bit flip from a solar flare.{1}".format(sys.argv[0], e))
...
def send_email(cfg, links):
    ...
    e_c = EmailCreds(name=cfg["User"]["name"], email=cfg["User"]["email"], passwd=cfg["User"]["email_passwd"])
    if any(credential == "" for credential in list(e_c)):
        ...
        """.format(len(links), '\n'.join("{0}. {1}".format(idx, link) for idx, link in enumerate(links, 1))
        sys.exit(logger.warning("One of the email User credentials is empty in config.ini. Please reconcile and re-run. Exiting...")))
